# Olá, eu sou o Thiago! 👋

Estudante de **Ciência da Computação**, focado em desenvolvimento backend e construção de soluções eficientes. Atualmente estou aprofundando meus conhecimentos em **Java**, **Orientação a Objetos** e **Estrutura de Dados**, além de desenvolver projetos práticos para consolidação do aprendizado.

---

### 💻 Tecnologias & Ferramentas

- **Linguagens Principal:** Java, JavaScript
- **Web / Frontend:** HTML5, CSS3
- **Controle de Versão:** Git & GitHub
- **IDE / Ferramentas:** VS Code, IntelliJ IDEA

---

### 📚 O que estou praticando no momento?

- ☕ Formação Java na Alura (Sintaxe, POO, Estruturas de Dados e Coleções)
- 🛠️ Boas práticas de código, versionamento com Git e arquitetura limpa
- 🎓 Projetos acadêmicos e desafios de lógica de programação

---

### 📊 Estatísticas do GitHub

### 💻 Tecnologias & Ferramentas

![Estatísticas do GitHub](https://github-readme-stats.vercel.app/api?username=tthiago-oliveira&show_icons=true&theme=dark&include_all_commits=true&count_private=true)

![Linguagens mais usadas](https://github-readme-stats.vercel.app/api/top-langs/?username=tthiago-oliveira&layout=compact&theme=dark)

---

🤝 **Vamos nos conectar?**
- [LinkedIn](https://www.linkedin.com/in/thiago-da-silva-oliveira-91ba47359/)
- Email: `thiago.oliveira.tech@gmail.com`

![Snake animation](https://github.com/tthiago0-oliveira/tthiago-oliveira/blob/output/github-contribution-grid-snake.svg)
